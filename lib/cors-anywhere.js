...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (corsAnywhere.originBlacklist.indexOf(ip) >= 0) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + ip + '" was blacklisted by the operator of this proxy.');
      return;
    }

    if (corsAnywhere.originWhitelist.length && corsAnywhere.originWhitelist.indexOf(ip) === -1) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + ip + '" was not whitelisted by the operator of this proxy.');
      return;
    }

    var rateLimitMessage = corsAnywhere.checkRateLimit && corsAnywhere.checkRateLimit(ip);
    if (rateLimitMessage) {
      res.writeHead(429, 'Too Many Requests', cors_headers);
      res.end('The origin "' + ip + '" has sent too many requests.\n' + rateLimitMessage);
      return;
    }

    if (corsAnywhere.redirectSameOrigin && ip && location.href[ip.length] === '/' &&
        location.href.lastIndexOf(ip, 0) === 0) {
